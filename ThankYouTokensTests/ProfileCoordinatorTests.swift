...
class ProfileCoordinatorTests: XCTestCase {
    ...
    let coord = ProfileCoordinator(navigationController: UINavigationController())
    ...
    func test_check_home_viewcontroller() {
        coord.start()
        XCTAssertTrue(coord.navigationController.topViewController is ProfileViewController)
    }
