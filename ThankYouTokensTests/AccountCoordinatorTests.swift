...
class AccountCoordinatorTests: XCTestCase {
    ...
    let coord = AccountCoordinator(navigationController: UINavigationController())
    ...
    func test_check_home_viewcontroller() {
        coord.start()
        XCTAssertTrue(coord.navigationController.topViewController is AccountViewController)
    }
